...
so this finishes the rename everywhere else), raises the minimum platform
requirements, and adds several new opt-in features. There are no
behavioral changes to how images are transformed — a v3 image URL and its
v4 equivalent produce the same pixels.
...
### 8. `ntzrbtr/flysystem-http` — no action needed
...
### 9. Review the new `strip` EXIF option (optional)
